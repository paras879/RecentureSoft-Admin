...
export async function checkPermission(permissionKey, accessType = 'write') {
    try {
        const cookieStore = await cookies();
        const token = cookieStore.get("admin_token")?.value;
        ...
        if (!adminDoc || !adminDoc.permissions) return true; // Default allow

        const permission = adminDoc.permissions.get(permissionKey);
        if (!permission) return true; // Default allow

        return permission[accessType] !== false;
    } catch (e) {
